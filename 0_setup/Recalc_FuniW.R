...
ids_filt = read.table("./input_dfs/RingIdsCov5.txt", sep = ',')[[1]]
...
for(ss in SuperScaffolds){
 ...
 # bed_filt=select.inds(bed, id%in%ids_filt)# filtering out the med / high cov ids 
...
write.table(Funi_recalc,
            file="./outputs/FuniW_goodQ_ss_only_subset.txt",
